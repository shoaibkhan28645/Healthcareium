...
export default function ServicesCards() {
  return (
    <div className="pb-5 px-6 md:px-12 lg:px-16" style={{ backgroundColor: "#001D21" }}>
      <div>
        <h1 className="bg-text-gradient text-center font-medium font-chillax text-5xl bg-clip-text text-white mb-14">
          Our Services
        </h1>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
          {servicesData.map((service, index) => (
            <div key={index} className="group">
              ...
              {/* Wrapped clickable content in Link */}
              <Link href={service.href}>
                <div className="relative gap-2 rounded-2xl overflow-hidden mb-6 border border-purple-200 shadow-lg hover:scale-[1.02] transition-transform duration-300">
                  <Image
                  ...
                  <div className="absolute bottom-4 right-4">
                    <div className="w-12 h-12 bg-black/20 cursor-pointer backdrop-blur-sm rounded-full flex items-center justify-center border border-white/10">
                      <svg
                        className="w-5 h-5 transition-transform duration-400 ease-out hover:scale-125 text-white"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
